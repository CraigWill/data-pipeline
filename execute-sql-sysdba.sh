#!/bin/bash
# 以 SYSDBA 权限在 Oracle 容器中执行 SQL 脚本
...
# 清理临时文件
docker exec oracle11g rm -f /tmp/script.sql 2>/dev/null || true
